...
High_HO_break_SRAR <- foreach(
  r = 1:R,
  .packages = c(
    "RobSparseMVA"
  )
) %dopar% {
  res_normal <- simulation_function(r, "HIGH_HO", "NORMAL", 0)
  res_cont1 <- simulation_function(r, "HIGH_HO", "CONTAMINATED", 0.1)
  res_cont2 <- simulation_function(r, "HIGH_HO", "CONTAMINATED", 0.2)
  res_cont3 <- simulation_function(r, "HIGH_HO", "CONTAMINATED", 0.3)
  res_cont4 <- simulation_function(r, "HIGH_HO", "CONTAMINATED", 0.4)
  res_cont5 <- simulation_function(r, "HIGH_HO", "CONTAMINATED", 0.5)
...
save(High_HO_break_SRAR, file = file.path("results", "high_ho_break_SRAR.Rdata"))
