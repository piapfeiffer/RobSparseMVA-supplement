# The functions below have been provided by
# Mai Q, Zhang X. An iterative penalized least squares approach to sparse canonical correlation analysis. 
# Biometrics. 2019 Sep;75(3):734-744. doi: 10.1111/biom.13043. Epub 2019 Apr 9. PMID: 30714093.
# The function "cv.SCCA.equal" was adapted to also be applicable to higher-order correlations for comparison.
...
SCCA <- function(x, y, alpha.init = NULL, beta.init = NULL, lambda.alpha, lambda.beta, niter = 100, npairs = 1, init.method = c("sparse","uniform","svd","random"), alpha.current = NULL, beta.current = NULL, standardize = TRUE, eps=1e-4){
  ...
  for(ipairs in (npairs0 + 1):npairs){
    ...
    obj <- SCCA.solution(x = x, y = y, x.Omega = x.tmp, y.Omega = y.tmp, alpha0 = alpha0, beta0 = beta0, lambda.alpha = lambda.alpha0, lambda.beta = lambda.beta0, niter = niter, eps = eps)
...
init1 <- function(sigma.YX.hat, sigma.X.hat, sigma.Y.hat, init.method, npairs, npairs0, alpha.current, beta.current, n=n, eps = 1e-4, d = NULL){
  ...
  if(init.method == "svd"){
    ...
    beta.init <- obj.svd$v[ , npairs0 + 1, drop = F]
  }
  if(init.method == "uniform"){
...
# The function cv.SCCA.equal is the cross validation function for the first and second canonical pair, assuming that alpha and beta uses the same tuning parameter.
# This function was adapted from the original one provided in the supplementary material of: 
# Mai Q, Zhang X. An iterative penalized least squares approach to sparse canonical correlation analysis. 
# Biometrics. 2019 Sep;75(3):734-744. doi: 10.1111/biom.13043. Epub 2019 Apr 9. PMID: 30714093.
cv.SCCA.equal <- function(x, y, lambda, nfolds=5, init.method = "svd", npairs = 1, eps = 1e-3, niter = 20){
  n <- nrow(x)
  p <- ncol(x)
  q <- ncol(y)
  ...
  for (np in 1:npairs){
    for(i.lambda in 1:length(lambda)){
      for(i in 1:nfolds){
        sigma.Y.hat <- cov(y[id.folds!=i, ])
        sigma.X.hat <- cov( x[id.folds!=i, ]) 
        sigma.YX.hat <- cov(y[id.folds!=i, ],  x[id.folds!=i, ])
        
        if (np == 1){
          obj.init <- init0(sigma.YX.hat = sigma.YX.hat, sigma.X.hat = sigma.X.hat, sigma.Y.hat = sigma.Y.hat, npairs = 1, n = n, init.method = init.method)
          alpha.init <- obj.init$alpha.init
          beta.init <- obj.init$beta.init
          ...
          alpha.current <- obj$alpha
          beta.current <- obj$beta
        } else {
          obj.init <- init1(sigma.YX.hat = sigma.YX.hat, sigma.X.hat = sigma.X.hat, sigma.Y.hat = sigma.Y.hat, npairs = np, npairs0 = np-1, alpha.current = alpha.current, beta.current = beta.current, 
                            n = n, init.method = init.method)
          alpha.init <- obj.init$alpha.init
          beta.init <- obj.init$beta.init
          obj <- SCCA(x[id.folds!=i, ], y[id.folds!=i, ], alpha.init = alpha.init, beta.init = beta.init, 
                      lambda.alpha = c(bestlambdas[1:(np-1)],lambda[i.lambda]), lambda.beta = c(bestlambdas[1:(np-1)],lambda[i.lambda]), 
                      eps = eps, niter = niter)
        }
        rho[i.lambda, i]<- cor(x[id.folds==i, ]%*% obj$alpha, y[id.folds==i,] %*% obj$beta)
      }
    }
    bestlambdas[np] = lambda[which.max(apply(rho, 1, mean))]
    if (np > 1){
      obj_best <- SCCA(x[id.folds!=i, ], y[id.folds!=i, ], alpha.init = alpha.init, beta.init = beta.init, 
                       lambda.alpha = bestlambdas[1:np], lambda.beta = bestlambdas[1:np], 
                       eps = eps, niter = niter)
      alpha.current <- cbind(alpha.current, obj_best$alpha)
      beta.current <- cbind(beta.current, obj_best$beta)
    }
  }
  
  list(rho = rho, lambda = lambda, bestlambda = bestlambdas)
}