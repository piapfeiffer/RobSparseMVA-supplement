# File to replicate results for low-dimensional setting
# ======================================================
    ...
    try(
      {
        ...
        TPR_a <- RobSparseMVA::TPR(as.matrix(true_CCA$a[, 1:order]), as.matrix(res_CCA$a))
        TPR_b <- RobSparseMVA::TPR(as.matrix(true_CCA$b[, 1:order]), as.matrix(res_CCA$b))
...
Low_HO <- foreach(
  ...
  kwargs <- list(Pearson = NA, Spearman = NA, Kendall = NA, MRCD = c(alpha = 0.75), OGK = NA)
  res_normal <- simulation_function(r, "LOW_HO", "NORMAL", methods, 0, kwargs)
  res_cont <- simulation_function(r, "LOW_HO", "CONTAMINATED", methods, 0.05, kwargs)
  res_tdist <- simulation_function(r, "LOW_HO", "T_DIST", methods, 0, kwargs)
...
save(Low_HO, file = file.path("results", "low_ho_orthstart.Rdata"))
